...
`;console.info("%c  SMART-ALARM-CARD  %c  v0.2.0  ","color: white; background: #0288d1; font-weight: 700","color: #0288d1; background: white; font-weight: 700");const Et=window;Et.customCards=Et.customCards||[],Et.customCards.push({type:ft,name:"Smart Alarm Card",description:"Bedside-clock style control for a Smart Alarm.",preview:!1});const Ct="07:00";let kt=class extends at{constructor(){super(...arguments),this._selectedDay=function(t){return(t.getDay()+6)%7}(new Date),this._now=new Date,this._onToggleEnable=()=>{this._call(this._state()===vt?"enable":"disable")},this._onSelectDay=t=>{this._selectedDay=t},this._onToggleSelectedDay=()=>{const t=this._selectedDay;this._dayTime(t)?this._call("clear_day",{day:t}):this._call("set_day_time",{day:t,time:Ct})},this._onHour=t=>{const e=parseInt(t.target.value,10),i=(this._dayTime(this._selectedDay)??Ct).split(":")[1];this._call("set_day_time",{day:this._selectedDay,time:`${String(e).padStart(2,"0")}:${i}`})},this._onMinute=t=>{const e=parseInt(t.target.value,10),i=(this._dayTime(this._selectedDay)??Ct).split(":")[0];this._call("set_day_time",{day:this._selectedDay,time:`${i}:${String(e).padStart(2,"0")}`})},this._onTestNow=()=>this._call("trigger_now"),this._onSnooze=()=>this._call("snooze"),this._onStop=()=>this._call("stop")}static async getConfigElement(){return await Promise.resolve().then(function(){return Dt}),document.createElement(yt)}static getStubConfig(){return{type:`custom:${ft}`,entity:""}}setConfig(t){if(!t?.entity)throw new Error("entity is required");this._config=t}getCardSize(){return 5}connectedCallback(){super.connectedCallback(),this._clockTimer=window.setInterval(()=>{this._now=new Date},1e3)}disconnectedCallback(){super.disconnectedCallback(),this._clockTimer&&window.clearInterval(this._clockTimer)}_use12h(){return"12h"===this._config?.time_format}_state(){return this.hass?.states[this._config.entity]?.state}_attrs(){return this.hass?.states[this._config.entity]?.attributes||{}}_name(){if(this._config?.name)return this._config.name;const t=this.hass?.states[this._config.entity]?.attributes?.friendly_name;return t||this._config?.entity||"Alarm"}_schedule(){return this._attrs().schedule??{}}_dayTime(t){return this._schedule()[String(t)]}_fmtTime(t){const[e,i]=t.split(":").map(t=>parseInt(t,10));if(this._use12h()){const t=e<12?"AM":"PM";return`${e%12==0?12:e%12}:${String(i).padStart(2,"0")} ${t}`}return`${String(e).padStart(2,"0")}:${String(i).padStart(2,"0")}`}_formatNextTrigger(t){if(!t)return"—";const e=new Date(t);if(Number.isNaN(e.getTime()))return"—";return`${e.toLocaleDateString(void 0,{weekday:"short",day:"2-digit",month:"short"})} · ${e.toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit",hour12:this._use12h()})}`}_call(t,e={}){this.hass.callService($t,t,{entity_id:this._config.entity,...e})}render(){if(!this._config||!this.hass)return W;const t=this._state();if(void 0===t)return q`<ha-card><div class="header">Entity not found</div></ha-card>`;const e=this._attrs(),i=t===bt||t===At,s=gt({[`state-${t}`]:!0});return q`
      <ha-card class=${s}>
        ${this._renderClock(t)}
    ...
    `}_renderDays(){return q`
      <div class="day-row">
        ${wt.map((t,e)=>{const i=this._dayTime(e),s=gt({day:!0,selected:e===this._selectedDay,active:!!i});return q`
            <div
              class=${s}
              title=${xt[e]}
              role="button"
    ...
    `}_renderEditor(){const t=this._selectedDay,e=this._dayTime(t),i=!!e,[s,r]=(e??Ct).split(":").map(t=>parseInt(t,10));return q`
      <div class="editor">
        ...
        ${i?q`
              <div class="slider-row">
                ...
                <input
                  ...
                  @input=${this._onHour}
                />
                <span class="slider-value">${this._fmtTime(e)}</span>
              </div>
              <div class="slider-row">
                ...
                />
                <span class="slider-value">${String(r).padStart(2,"0")}</span>
              </div>
  ...
  `,t([pt({attribute:!1})],Mt.prototype,"hass",void 0),t([ut()],Mt.prototype,"_config",void 0),Mt=t([lt(yt)],Mt);var Dt=Object.freeze({__proto__:null,get SmartAlarmCardEditor(){return Mt}});export{kt as SmartAlarmCard};
